...
"""
function fly_through(volume::AbstractArray;
                     dim::Int=3,
                     colormap=nothing,
                     db_scale::Bool=false)
...
Plot figure (or `nothing` without a Makie backend).
"""
function overlay_plot(background::AbstractArray, overlay::AbstractArray;
                      alpha::Real=0.5,
...
Plot figure (or `nothing` without a Makie backend).
"""
function stacked_plot(signals::AbstractArray;
                      dt::Real=1.0,
